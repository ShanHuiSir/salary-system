...
function safeDivide(numerator: number, denominator: number, precision = 2): number {
  return denominator === 0 ? 0 : round(numerator / denominator, precision);
}

function ratio(numerator: number, denominator: number, precision = 2): number {
  return denominator === 0 ? 0 : round((numerator / denominator) * 100, precision);
}
...
export function autoFillOverviewRevenueFields(values: Record<string, unknown>): Record<string, unknown> {
  ...
  if (!totalIsBlank && missingComponentKeys.length === 1) {
    ...
    next[missingComponentKeys[0]] = round(toNumber(next.totalRevenue) - knownSum, 2);
  }

  if (totalIsBlank && missingComponentKeys.length === 0) {
    next.totalRevenue = round(REVENUE_COMPONENT_KEYS.reduce((sum, key) => sum + toNumber(next[key]), 0), 2);
  }
...
export function normalizeSalaryData(data: PersistedSalaryData): PersistedSalaryData {
  ...
  const overviews = overviewsBase.map((overview) => {
    ...
    return {
      ...
      laborCostRatio: ratio(overview.totalLaborCost, overview.totalRevenue, 2),
      perCapitaRevenue: safeDivide(overview.totalRevenue, headcount, 2),
      momRevenue: round(overview.totalRevenue - (previous?.totalRevenue ?? 0), 2),
      yoyRevenue: round(overview.totalRevenue - (lastYear?.totalRevenue ?? 0), 2),
      momLaborCost: round(overview.totalLaborCost - (previous?.totalLaborCost ?? 0), 2),
      yoyLaborCost: round(overview.totalLaborCost - (lastYear?.totalLaborCost ?? 0), 2),
    };
  });
  ...
  const departments = departmentsBase.map((department) => {
    ...
    return {
      ...
      laborCostRatio: ratio(department.laborCost, revenue, 2),
      momLaborCost: round(department.laborCost - (previous?.laborCost ?? 0), 2),
      yoyLaborCost: round(department.laborCost - (lastYear?.laborCost ?? 0), 2),
      momHeadcount: round(department.headcount - (previous?.headcount ?? 0), 2),
      yoyHeadcount: round(department.headcount - (lastYear?.headcount ?? 0), 2),
    };
  });
  ...
  const stores = storesBase.map((store) => {
    ...
    return {
      ...
      momRevenue: round(store.revenue - (previous?.revenue ?? 0), 2),
      yoyRevenue: round(store.revenue - (lastYear?.revenue ?? 0), 2),
    };
  });
  ...
  const positions = positionsBase.map((position) => {
    ...
    return {
      ...position,
      ratio: ratio(position.headcount, totalHeadcount, 2),
      momHeadcount: round(position.headcount - (previous?.headcount ?? 0), 2),
      yoyHeadcount: round(position.headcount - (lastYear?.headcount ?? 0), 2),
    };
  });
